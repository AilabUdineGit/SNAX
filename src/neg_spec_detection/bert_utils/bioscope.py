...
class Bioscope():
    def __init__(self, task, train_size = 0.8):
        ...
        if task not in ['negation', 'speculation']:
            raise Exception("Must be 'negation' or 'speculation'")
        self._sentences = pd.read_pickle(os.path.join("..", "datasets", "bioscope", "bioscope_sentences.pickle"))
        all_scope       = pd.read_pickle(os.path.join("..", "datasets", "bioscope", "bioscope_scopes.pickle"))
        ...
        df_neg = Tagger(new_df[['text_id','text']],new_df,"").get().rename(columns={'text_id': 'id', 'text': 'sentence', 'tokens': 'spacy_tokens'}).drop(columns=['biluo_tags', 'io_tags']).sample(frac=-1).sample(2800)
        ...
        scope_spec = scope_spec[~scope_spec.id.isin(df_neg.id.values)].sample(390)
